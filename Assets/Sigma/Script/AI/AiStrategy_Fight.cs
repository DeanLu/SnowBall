...
{	
	...
	public override void OnAnimatorIK(ref AiParam _param)
	{
		...
		{		
			...
			_param.Anim.SetLookAtPosition (_param.ObjTarget.transform.position + _param.ObjTarget.transform.up);
	...
	void ShootBall(ref AiParam _param)
	{
		...
		Vector3 shootDir = _param.Owner.transform.forward + _param.Owner.transform.up * 0.5F;
		...
		Physics.IgnoreCollision(col, _param.Owner.GetComponent<Collider>(), true);
